use {
    crate::{assert_unchanged, test_instructions, utils},
    maplit::hashmap,
    perpetuals::{
...
pub async fn open_and_close_long_position_accounting() {
...

    {
        ...
        // Double check effect of opening position on ETH custody accounting
        {
            ...
            // Distributed rewards
            {
                let before = &eth_custody_account_before.distributed_rewards;
                let after = &eth_custody_account_after.distributed_rewards;

                assert_eq!(
                    before.open_position_lm + 50_000,
                    eth_custody_account_after
                        .distributed_rewards
                        .open_position_lm
                );

                assert_unchanged!(before.swap_lm, after.swap_lm);
                assert_unchanged!(before.add_liquidity_lm, after.add_liquidity_lm);
                assert_unchanged!(before.remove_liquidity_lm, after.remove_liquidity_lm);
                assert_unchanged!(before.close_position_lm, after.close_position_lm);
                assert_unchanged!(before.liquidation_lm, after.liquidation_lm);
            }
            ...
            // Volume stats
            {
                ...
                assert_eq!(
                    // lm_staker_fee share of expected_protocol_fee is 14_962_500, multiplied by 1,500 (eth price)
                    before.swap_usd + 22_443_750,
                    after.swap_usd
                );
...

    {
        ...
        // Check user balance
        {
            ...
            // entry_position_price - exit_price (both taking into account spread)
            //
            // Fees: $75,750001
            ...
            assert_eq!(
                martin_eth_ata_balance_before + 393_888_877,
                martin_eth_ata_balance_after
            );
        }
        ...
        // Double check effect of closing position on ETH custody accounting
        {
            ...
            // Distributed rewards
            {
                ...
                assert_eq!(before.close_position_lm + 56_111, after.close_position_lm);

                assert_unchanged!(before.open_position_lm, after.open_position_lm);
                assert_unchanged!(before.swap_lm, after.swap_lm);
                assert_unchanged!(before.add_liquidity_lm, after.add_liquidity_lm);
                assert_unchanged!(before.remove_liquidity_lm, after.remove_liquidity_lm);
                assert_unchanged!(before.liquidation_lm, after.liquidation_lm);
            ...
            // Trade Stats
            {
                ...
                assert_eq!(before.loss_usd + 968_250_016, after.loss_usd);
            ...
            // Long positions
            {
                ...
                assert_eq!(
                    before.cumulative_interest_snapshot + 2,
                    after.cumulative_interest_snapshot
                );
