...
pub struct IncreasePositionParams {
    ...
    pub collateral: u64,
    pub size: u64,
    // Value not used afterward, it uses the position.side. Parameters is there to make the intent clearer
    pub side: Side,
}

// Add size to a position
pub fn increase_position(
    ...
    // must increase position in the same collateral and side
    require!(
        ctx.accounts.position.collateral_custody == collateral_custody.key(),
        PerpetualsError::InvalidCollateralCustody
    );
    require!(
        ctx.accounts.position.side == params.side,
        PerpetualsError::InvalidCollateralCustody
    );

    // validate inputs
    msg!("Validate inputs");
    if params.price == 0 || params.size == 0 || side == Side::None {
        return Err(ProgramError::InvalidArgument.into());
    }

    let use_collateral_custody = side == Side::Short || custody.is_virtual;
    ...
    let position_price = pool.get_entry_price(&token_price, &token_ema_price, side, custody)?;
    ...
    let locked_amount = if use_collateral_custody {
        ...
        custody.get_locked_amount(params.size, side)?
    };
    ...
    // update position
    // calculate the new position price (weighted average of the old and new prices)
    let new_position_price = math::checked_div(
        math::checked_add(
            math::checked_mul(position_price, position.size_usd)?,
            math::checked_mul(params.price, size_usd)?,
        )?,
        position.size_usd + size_usd,
    )?;

    msg!("Increase existing position");
    position.update_time = curtime;
    position.price = new_position_price;
    position.size_usd = math::checked_add(position.size_usd, size_usd)?;
    position.borrow_size_usd = math::checked_add(position.borrow_size_usd, borrow_size_usd)?;
    ...
    position.locked_amount = math::checked_add(position.locked_amount, locked_amount)?;
